...
class ServerThread(threading.Thread):
    ...
    DirNam1 file1 file2 file3\n
    DirNam2 file1 file10 file20\n
    '''
    def send_manifest(self, recipient_conn):
        ...
        for directory in manifest.keys(): # iterate through contents, put then into string form
            line = directory
            dir_files = manifest[directory]
        ...
        logging.debug("We have successfully sent the manifest")
    ...
    def serve_file(self, conn, filepath):
        logging.debug("Serving file {}".format(filepath))
        ...
        logging.debug("Finished serving file {}".format(filepath))
    ...
    def server(self, conn, client_ip, network_size=None):
        logging.debug("Entering the server function in ServerThread")
        data = conn.recv(2048)  # expected max ~ 1500 bytes
        logging.debug("We received the message {}".format(data.decode()))
        if data:
            logging.debug("Entering the decision in ServerThread")
            reply = None  # response message to client
            ...
            if "JOIN" in msg:
                ...
                if self.node.getNodeID() == 0:
                    ...
                    logging.debug("Sending this reply to client:", reply)
                    ...
                    self.network_size += 1
                    logging.debug("We successfully allowed the new node to join our overlay network")
                else:
                    logging.debug("We are not node 0, we aren't serving the request")
                    # redirect supplicant to the next node in the list
                    reply = "REDIRECT {} -1 -1".format(self.node.getNext())
                ...
                if msg.split()[1] == "NEXT":
                    ...
                    logging.error("ERROR WE HAVE AN ERROR UPDATING OUR NODE INFORMATION".format(msg))
                ...
                logging.debug("THE REQUEST RECEIVED IS UNKNOWN:", msg)
        logging.info("finished serving client, continuing to listen for more connections :)")
    ...
    Design informed by the python cookbook and 
    ...
    def run(self):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_listen:  # open a socket for IPv4 TCP traffic
            ...
            while not self._stopevent.isSet():
                try:
                    # block while waiting for new connections, these will be represented by the conn object
                    conn, addr = server_listen.accept()
